  ...
  return (
    <div style={{ textAlign: "center" }}>
      ...
      <form onSubmit={handleSubmit} style={{ marginBottom: "2rem" }}>
        <input type="text" value={nama} onChange={(e) => setNama(e.target.value)} placeholder="Nama Dokter" style={{ marginRight: "1rem" }} />
        <input type="text" value={harga} onChange={(e) => setHarga(e.target.value)} placeholder="Spesialis" style={{ marginRight: "1rem" }} />
        <input type="text" value={kategori} onChange={(e) => setKategori(e.target.value)} placeholder="Alamat" style={{ marginRight: "1rem" }} />
      ...
      <table style={{ margin: "0 auto", borderCollapse: "collapse" }}>
        <thead>
          <tr>
            <th style={{ padding: "0.5rem", backgroundColor: "powderblue" }}>Nama Dokter</th>
            <th style={{ padding: "0.5rem", backgroundColor: "powderblue" }}>Spesialis</th>
            <th style={{ padding: "0.5rem", backgroundColor: "powderblue" }}>Alamat</th>
...
async function createMenu(menuData) {
  try {
    ...
    const response = await fetch("https://yue-be.vercel.app/menu", {
      method: "POST",
      headers: {
...
async function updateMenu(menuId, updateData) {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(`https://yue-be.vercel.app/menu/${menuId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
